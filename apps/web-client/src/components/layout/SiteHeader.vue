  ...
  <header class="sticky top-0 z-50 border-b border-neutral-200 bg-white/95 backdrop-blur-md">
    <div class="container-shell">
      <div class="flex items-center justify-between gap-6">
        ...
        <div class="hidden lg:flex lg:items-center">
          <div class="text-right">
            <div v-if="email" class="flex items-center justify-end gap-2 text-sm text-neutral-500">
              ...
              <a :href="`mailto:${email}`" class="transition-colors hover:text-primary-navy">
                {{ email }}
              </a>
            </div>
            <div class="mt-1 flex items-center justify-end gap-3 text-amber-500">
              <AppIcon icon="phone" class="h-6 w-6" :stroke-width="2.25" />
              <a v-if="phone" :href="`tel:${phone}`" class="text-lg font-extrabold transition-colors hover:text-amber-600 lg:text-xl">
                <span>{{ phone }}</span>
              </a>
            </div>
          </div>
        ...
        <button @click="emit('toggleMobileMenu')" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-neutral-200 text-neutral-600 hover:bg-neutral-100 hover:text-primary-navy lg:hidden" aria-label="Toggle menu">
