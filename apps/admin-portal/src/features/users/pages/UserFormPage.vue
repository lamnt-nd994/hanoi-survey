    ...
    <DataCard class="p-6">
      ...
      <form class="space-y-6" @submit.prevent="handleSubmit">
        <div class="grid gap-5 md:grid-cols-2">
          <FormField>
            <FormLabel>Tên đăng nhập <span class="text-rose-600">*</span></FormLabel>
            <Input v-model="form.username" placeholder="admin01" required />
          </FormField>

          <FormField>
            <FormLabel>Họ tên <span class="text-rose-600">*</span></FormLabel>
            <Input v-model="form.fullName" placeholder="Nguyễn Văn A" required />
          </FormField>
        ...
        <FormField>
          <FormLabel>Vai trò <span class="text-rose-600">*</span></FormLabel>
        ...
        <div class="flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <div class="flex gap-3">
            <Button as="router-link" to="/users" variant="secondary">Quay lại</Button>
            <Button type="submit" :disabled="saving">{{ saving ? 'Đang lưu...' : (isEdit ? 'Cập nhật người dùng' : 'Tạo người dùng') }}</Button>
          </div>
        </div>
      </form>
    </DataCard>
